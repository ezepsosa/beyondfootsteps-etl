...
class IdpIdmcJob(GoldJob):
    ...
    def run(self, configuration: Configuration, args: Namespace):
        ...
        df_displacement = df_idpreturnees.alias('idpreturnees').join(
            df_population.alias('population'),
            on=[(col('population.year') == col('idpreturnees.year')) & (col('idpreturnees.country_of_origin_iso') == col('population.country_code'))],
            how='left').withColumn('displacement_rate_per_100k', try_divide(col('idpreturnees.total'), col('population.population')) * lit(100000)) \
                .select(col('idpreturnees.id_idpidmc'),
